...
wikiext は高性能なストリーミング Wikipedia ダンプテキスト抽出ツールとして設計されています。ストリーミング I/O とバッチベースの並列処理を組み合わせて、数十 GB の XML ダンプを処理します。
...
- **ライブラリファースト** -- コア機能は `wikiext` ライブラリクレートに集約。CLI は薄いラッパー
